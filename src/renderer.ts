...
const CSS = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .tag { display: inline-block; background: #e9ecef; color: #495057; border-radius: 4px; padding: 2px 8px; font-size: 12px; margin: 2px 2px 0 0; }
  ...
  .badge { display: inline-flex; align-items: center; padding: 3px 10px; border-radius: 12px; font-size: 12px; font-weight: 600; margin-top: 10px; }
...
function trustIQBadge(score: number | null): string {
...
function renderCard(supplier: Supplier): string {
  ...
  const address = addressParts.length ? `<div class="field">${esc(addressParts.join(', '))}</div>` : '';
...
function renderSummary(params: SearchParams, totalCount: number, resultCount: number): string {
  ...
  return `<div class="summary">
    ...
    <div class="meta">Showing ${resultCount} of ${totalCount} results${tags.length ? ' &middot; ' + tags.map((t) => `<span class="tag">${esc(t)}</span>`).join('') : ''}</div>
  </div>`;
}
